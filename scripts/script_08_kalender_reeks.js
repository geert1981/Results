...
function update_kalender_reeks_id() {
    // console.log('update_kalender_reeks_id fired ...');

    // OPHALEN KALENDERS ALLE TEAMS UIT REEKS & BEWAREN IN ARRAY_GAMES_ALL_TEAMS
    all_games = []; // LEEGMAKEN ARRAY OP NIEUW REQUEST
    
    let fetch_count = array_teams_reeks_guids.length;

    for (teller_teams = 0; teller_teams < fetch_count; teller_teams++) {
        team_id = array_teams_reeks_guids[teller_teams];
        // console.log(team_id);
        var link_kalender_reeks = 'http://vblcb.wisseq.eu/VBLCB_WebService/data/TeamMatchesByGuid?teamguid=';
        var url_kalender_reeks = link_kalender_reeks + team_id;
        // console.log('URL team:', url_kalender_reeks);
        const requestOptions_kalender_reeks = {
            method: 'GET',
            redirect: 'follow'
        };
        
        fetch(url_kalender_reeks, requestOptions_kalender_reeks)
        .then(response_kalender_reeks => response_kalender_reeks.json())
        .then(result_kalender_reeks => {
            // FILTER GAMES
            const filtered_games_kalender_reeks = filter_games_for_current_week(result_kalender_reeks);

            // ADD GAMES TO ALL_GAMES
            for (teller_filter = 0; teller_filter < filtered_games_kalender_reeks.length; teller_filter++){
                all_games.push(filtered_games_kalender_reeks[teller_filter]);
            }
        })
        .catch(error => console.log('error', error))
        .finally(() => {
            fetch_count--; // VERMINDER FETCHCOUNT ZODRA EEN FETCH IS VOLTOOID
            if (fetch_count === 0) {
                // ALLE FETCH OPROEPEN ZIJN VOLTOOID, VOER REST VAN CODE UIT
                setTimeout(() => {
                    // SORT GAMES
                    all_games.sort(compare_dates_times);
                    // FILTER DUBBELE GAMES ERUIT
                    const unique_all_games = filter_uniek_op_eigenschap(all_games, 'guid');
                    // console.log('Unique Games:', unique_all_games);
            
                    
                    // CREATE TABLE
                    const table_container_kalender_reeks = document.querySelector(".grid_kalender_reeks table tbody");
                    table_container_kalender_reeks.textContent = '';  // EMPTY CONTENT TBODY
                    for (const game_kalender_reeks of unique_all_games) {
                        // console.log('Adding game to table:', game_kalender_reeks);
                        const row = table_container_kalender_reeks.insertRow();
                        // PUT DATA IN VAR's
