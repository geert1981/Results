...
async function update_club_details() {
    ...
    var stamnummer_site_element = document.getElementById('stamnummer_site');
    stamnummer_site_element.innerHTML = '';
    ...
    // console.log('URL Club details:', url_club_details);
    ...
    fetch(url_club_details, requestOptions_club_details)
        ...
        .then(result_club_details => {
            ...
            var club_naam = result_club_details[0].naam;
            var club_id = result_club_details[0].guid;
            var club_stamNr = result_club_details[0].stamNr;
            var club_website = result_club_details[0].website; 
            var club_adres_straat = result_club_details[0].adres.straat; 
            var club_adres_nr = result_club_details[0].adres.huisNr; 
            var club_adres_postcode = result_club_details[0].adres.postcode; 
            var club_adres_plaats = result_club_details[0].adres.plaats;
            var club_email = result_club_details[0].email; 
            

            // BUTTONS SPORTHALLEN
            let aantal_sporthallen = result_club_details[0].accomms.length;
            // console.log('Aantal sporthallen:', aantal_sporthallen);

            var sporthallen_container = document.getElementById('sporthallen_buttons');
            sporthallen_container.innerHTML = ''; // EMPTY CONTAINER BEFORE ADDING BUTTONS
